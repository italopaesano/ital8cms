...
- v2.30.0 (2026-06-20): **DOCS** - Documentazione HTTPS riorganizzata attorno alle due architetture di produzione e nuovo deep-dive teorico, con focus NixOS. Modifiche:
  ...
  - **Nuovo `docs/EXPLAIN-https.it.md` (rev 1) + stub `docs/EXPLAIN-https.md`:** deep-dive che parte dalla teoria (cos'è un certificato HTTPS, X.509, catena di fiducia, handshake, tipi; come si generano e si rinnovano periodicamente: CSR/CA tradizionale, ACME RFC 8555, challenge HTTP-01/DNS-01/TLS-ALPN-01, Let's Encrypt 90 giorni, client lego/certbot) e applica la teoria alla **soluzione reverse proxy** (Strada A) in generale e su NixOS (modulo `ital8cms-proxy.nix`). Include la nota architetturale su `app.proxy` non impostato in `index.js` (header `X-Forwarded-*`, IP client, schema, cookie `Secure`).
