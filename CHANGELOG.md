...
- v2.51.0 (2026-07-11): **CHORE (dependencies)** - Aggiornato `koa-classic-server` `^3.0.0` (lock a 3.0.1) → `^4.0.0`. Nessuna modifica di codice: l'API usata dalle 5 istanze in `index.js` è invariata (la firma di `template.render` guadagna 2 argomenti opzionali in coda, `rawBuffer` e `signal`, retrocompatibili; in futuro `signal` potrà essere inoltrato a `ejs.renderFile` per onorare il `renderTimeout`).
  - **Breaking change upstream adottata — trailing slash canonico** (default `dirListing.trailingSlash: true`): `GET /dir` (directory senza slash) → **301 → `/dir/`** (query string preservata) e `GET /file/` (file con slash finale) → **404**; in v3 entrambi rispondevano 200. Default **mantenuto** (nessun opt-out): fa risolvere correttamente i link relativi nelle pagine indice. Impatti verificati: la sitemap del plugin `seo` emette già URL di directory **con** slash (`pathToUrl`) → il serving ora coincide con la sitemap; il menu admin usa URL con file esplicito (`/admin/{sezione}/index.ejs`) → nessun hop di redirect; `/admin` senza slash per i non autenticati resta intercettato prima dall'access control (302 login, invariato).
  - **Altre novità v4 (nessuna azione richiesta):** `compression.maxFileSize` (10 MB) — i file più grandi vengono compressi in streaming a RAM limitata invece che bufferizzati interi; validazione più severa (`options` non-oggetto → throw); conformance HTTP (q-values di `Accept-Encoding`, `Vary`, precedenza `If-None-Match`/`Range`). La 3.1.0 intermedia (inclusa nel salto) ha introdotto la policy `symlinks`, default `'follow'` — **essenziale** per le istanze che servono sezioni admin e pluginPages via symlink: non impostare mai `'follow-within-root'` su quelle istanze.
  - **Nota per la futura v5 upstream (residuo noto, stessa famiglia della v2.45.0):** le istanze theme-resources (×2), pluginPages e admin passano ancora `urlsReserved` costruiti assoluti (`${globalPrefix}/${prefix}`) anziché **relativi** a `urlPrefix`. Col default `globalPrefix=""` le entry sono formalmente valide (nessun warning al boot, verificato); con `globalPrefix` valorizzato diventano multi-segmento: già oggi non matchano (config morta), la v4 emette deprecation warning e la **v5 lancerà errore**. Da ripulire in un intervento dedicato.
  - **Verifica:** suite jest completa **97 suite / 2469 test** verde sia con 3.0.1 (baseline) sia con 4.0.0; boot pulito senza warning di deprecazione; A/B dal vivo su server reale (`/media` → 301 `/media/` con query preservata, `/robots.txt/` → 404; rotte `/api/**`, risorse tema e pluginPages invariate).
  - **Files Modified:** `/package.json`, `/package-lock.json`, `/CLAUDE.md`, `/ital8Config.default.json5`, `/CHANGELOG.md`.
