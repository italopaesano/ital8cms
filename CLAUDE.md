...
| `rules.ejs` | **B + C — JSON5 editor + structured form** | `protectedRoutes.json5`; per-rule form (empty numeric = inherit) coordinated with the JSON5 editor (Validate / Save) |
...
- **Rules — View C (form) coordinated with View B (JSON5):** `rules.ejs` offers a per-rule form (one card per rule: `name`, `pathPattern`, 6 override fields where empty = inherit defaults) plus the raw JSON5 editor, with the same toggle pattern. Since `protectedRoutes.json5` is raw JSON5 (comments), the form is populated from `rules` **parsed server-side** (`GET /rules` returns both `content` and `rules`) — the browser needs no JSON5 parser. JSON→Form toggle uses `JSON.parse` and degrades gracefully (warns, stays on JSON5) when the textarea has comments. Saving from the form normalizes the file (comments lost); save from the JSON5 view to preserve comments.
...
**Changelog:**
- v2.11.2 (2026-06-02): **ENHANCEMENT** - `adminRateLimiter` Rules: added **View C (structured form)** coordinated with the existing **View B (raw JSON5 editor)**, completing the hybrid on every config page. `rules.ejs` now has a Form↔JSON5 toggle with a **per-rule card** form (`name`, `pathPattern`, and 6 override fields where empty = inherit the global defaults), an "Add rule"/"remove" UI, and the raw JSON5 editor over the same content. Since `protectedRoutes.json5` is raw JSON5 (with comments), the form is populated from **server-parsed `rules`**: `GET /rules` now returns both `content` (raw) and `rules` (parsed array), so the browser needs no JSON5 parser. The **textarea stays the single source of truth** sent to the server; Form→JSON regenerates via `JSON.stringify`, JSON→Form uses `JSON.parse` and degrades gracefully (warns + stays on JSON5) when the textarea contains comments. Dirty-tracking + `beforeunload`. Saving from the form normalizes the file (comments lost); save from the JSON5 view to keep comments. Server endpoints otherwise unchanged. Verified: JS syntax, EJS render of rules.ejs, plugin suite (34, incl. GET /rules `rules` assertion), full project suite green. **Files Modified:** `/plugins/adminRateLimiter/main.js` (GET /rules returns parsed `rules`), `/plugins/adminRateLimiter/adminWebSections/rateLimiterManagement/{rules.ejs,rules-editor.js}`, `/plugins/adminRateLimiter/tests/unit/routes.test.js`, `/plugins/adminRateLimiter/{README.md,EXPLAIN.md}`, `/CLAUDE.md`.
