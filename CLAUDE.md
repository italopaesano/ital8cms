...
3. **Ciclo di vita config** (prima del caricamento plugin/temi — vedi [`docs/decisions/config-lifecycle.it.md`](./docs/decisions/config-lifecycle.it.md)):
   ...
   - **Materializza i due config vivi del core** (`materializeFromDefault`): `koaSession.json5` **prima** del montaggio dei priority middleware (che lo legge, a livello di modulo → serve la variante `materializeFromDefault.sync`), `adminConfig.json5` in `startApp()`. `ital8Config.json5` è **escluso di proposito**: la sua assenza è il gate `[INIT]`, e rigenerarlo scavalcherebbe il wizard. Se manca **anche** il `.default` non c'è nulla da cui rigenerare → box `[CONFIG]` + `exit 1`.
...
- `/core/materializeFromDefault.js` → `.sync` - Variante **sincrona** della materializzazione di una coppia, per i config letti prima che il boot entri in codice asincrono (`koaSession.json5`)
