...
- `getWritablePaths(pluginSys, pathPluginFolder)` - Dichiara le data dir che il plugin deve poter scrivere a runtime → `Array<{ path, purpose }>`. Consumata da `core/storageWritabilityCheck.js` in due contesti, entrambi con la stessa sonda effettiva (crea la dir + write/delete di un file temporaneo, che pre-crea anche le dir mancanti): (1) **gate di caricamento** (`assertPluginWritableOrThrow`, chiamato da `pluginSys` durante il caricamento di **ogni** plugin, **prima** di `installPlugin`/`loadPlugin`) — **GRACEFUL**: dir non scrivibile → box `[STORAGE]` "NON caricato" + `throw` → il catch graceful marca il plugin `incomplete`, lo rimuove dagli attivi e **non** persiste `isInstalled` a 1; il plugin è **saltato** ma il boot **prosegue** (un essenziale non caricato resta fatale via `#enforceEssentialPlugins`). Copre sia la prima installazione sia i plugin già installati la cui dir regredisce a non scrivibile (nessun preflight fatale separato). (2) **advisory wizard/offline** (`adviseWritabilityForPluginsDir`, in `scripts/init.js`) — NON bloccante, introspeziona i plugin senza caricarli, solo avviso precoce (il contesto del wizard può differire da quello del servizio a runtime). Opzionale; poiché il gate gira prima di `loadPlugin` (e il wizard non carica affatto i plugin), il metodo deve saper risolvere i path dal config via `pathPluginFolder` (vedi `analytics`).
