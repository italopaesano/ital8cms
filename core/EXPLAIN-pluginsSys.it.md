...
- `getWritablePaths(pluginSys, pathPluginFolder)` - Dichiara le data dir che il plugin deve poter scrivere a runtime → `Array<{ path, purpose }>`. Consumata da `core/storageWritabilityCheck.js` in tre contesti, tutti con la stessa sonda effettiva (crea la dir + write/delete di un file temporaneo, che pre-crea anche le dir mancanti): (1) **preflight di boot** (`checkStorageWritability`, dopo `pluginSys.initialize()`) — **BLOCCANTE**: dir non scrivibile → box `[STORAGE]` + `process.exit`; (2) **advisory install-time** (`advisePluginWritability`, in `pluginSys` alla transizione d'installazione, dopo `loadPlugin`) — NON bloccante; (3) **advisory wizard/offline** (`adviseWritabilityForPluginsDir`, in `scripts/init.js`) — NON bloccante, introspeziona i plugin senza caricarli. Gli advisory non bloccano perché il contesto d'install/wizard può differire da quello del servizio a runtime; il gate autorevole resta il boot. Opzionale; per l'uso offline il metodo dovrebbe saper risolvere i path dal config (vedi `analytics`).
