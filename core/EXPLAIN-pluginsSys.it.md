...
- `getWritablePaths(pluginSys, pathPluginFolder)` - Dichiara le data dir che il plugin deve poter scrivere a runtime → `Array<{ path, purpose }>`. Consumata da `core/storageWritabilityCheck.js` in tre contesti, tutti con la stessa sonda effettiva (crea la dir + write/delete di un file temporaneo, che pre-crea anche le dir mancanti): (1) **gate d'installazione** (`assertPluginWritableOrThrow`, in `pluginSys` alla transizione d'installazione, **prima** di `installPlugin`/`loadPlugin`) — **BLOCCANTE per quel plugin**: dir non scrivibile → box `[STORAGE]` "ANNULLATA" + `throw` → il catch graceful marca il plugin `incomplete` e **non** persiste `isInstalled` (plugin NON installato; il boot prosegue, salvo `essentialPlugins`); (2) **preflight di boot** (`checkStorageWritability`, dopo `pluginSys.initialize()`) — **BLOCCANTE (fatale)**: guardia sui plugin già installati, dir non scrivibile → box `[STORAGE]` + `process.exit`; (3) **advisory wizard/offline** (`adviseWritabilityForPluginsDir`, in `scripts/init.js`) — NON bloccante, introspeziona i plugin senza caricarli, solo avviso precoce (il contesto del wizard può differire da quello del servizio a runtime). Opzionale; per l'uso offline (gate d'installazione e wizard, dove `loadPlugin` non è ancora girato) il metodo deve saper risolvere i path dal config via `pathPluginFolder` (vedi `analytics`).
