 ...
 *
 * ─── PERCHE QUESTO TEST ESISTE (consolidamento C5) ────────────────────────────
 * I passi 7 e 8 hanno bumpato la `schemaVersion` di `pluginConfig.default.json5`
 * senza dichiarare uno step, convinti — a ragione — che il merge additivo del
 * boot bastasse: erano solo chiavi nuove dentro oggetti.
 *
 * Ma il runner non guarda se serva uno script, guarda se la catena COPRE IL
 * SALTO: `covered = pending.length === (target - liveVersion)`. Con il
 * descrittore a v5 e gli step fermi a v3 nessuna installazione poteva più
 * migrare — box `[MIGRATE]` di errore a ogni avvio, e
 * `npm run cli -- migrate sentinel` che si rifiutava di girare **inclusi i due
 * step veri**. Una catena bucata non ferma solo sé stessa.
 *
 * Il difetto è invisibile a mano — bisogna confrontare due file lontani — e si
 * ripresenta a ogni bump. Da qui questo test, che è un promemoria eseguibile:
 * chi bumpa la `schemaVersion` lo vede fallire e sa cosa gli manca.
 */
...
describe('catena delle migrazioni di sentinel', () => {
  ...
  // `reason` è obbligatorio ANCHE per gli step automatici senza script: dichiarare
  // che un salto è sicuro non è la stessa cosa che motivarlo, e questi due step
  // nascono proprio da un salto dato per sicuro e mai scritto da nessuna parte.
