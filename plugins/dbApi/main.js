...
function loadPlugin(){
  ...
  mainDb = new betterSqlite3(`${__dirname}/dbFile/mainDb.db`, { verbose: console.log });
  webDb = new betterSqlite3(`${__dirname}/dbFile/webDb.db`, { verbose: console.log });
  testDb = new betterSqlite3(`${__dirname}/dbFile/testDb.db`, { verbose: console.log });
};
...
function getObjectToShareToOthersPlugin( pluginName ){// pluginName = nome dell'altro plugin con cui sarà condiviso questo serve a creare comportamenti personalizati in base al plugin con cui si condivi
  ...
  return {
    db: new betterSqlite3(`${__dirname}/dbFile/pluginsDb/${pluginName}.db`, { verbose: console.log })// crea un file di database per ilplugin nell'a sottocartella dedicata :pluginsDB
  };
}
