...
**Confine di materializzazione** *(rivisto — vedi «Rigenerazione dei core al boot» più sotto)*:
- Il **wizard** (`npm run start-configure`) resta l'unico a **configurare** i core: le scelte non indovinabili (chiavi di sessione, utente root, profilo) nessun default le può inventare.
- Il **boot** materializza automaticamente i config di **plugin/tema** mancanti (default sani, zero interazione) **e i due config core rigenerabili** (`koaSession`, `adminConfig`).
- `ital8Config` resta **fuori**: la sua assenza è il gate di init qui sopra.
...
- **Rigenerazione dei core al boot (rivede il «confine di materializzazione» del §5):** il confine originale — wizard per i core, boot per plugin/temi — copriva il clone fresco ma non la vita successiva. I vivi dei core sono git-ignored e dichiarati «rigenerabili dai `.default`» come tutti gli altri, ma a crearli era **solo** il wizard: dopo l'installazione un `git clean -X` o un ripristino parziale da backup li faceva sparire per sempre, e l'avvio moriva con uno stack trace grezzo (`Failed to load admin config: file di configurazione non trovato`). Il boot ora materializza `koaSession.json5` e `adminConfig.json5`.
  - **Perché è sicuro proprio per questi due, e non per `ital8Config`:** il wizard fa *scelte*, il boot fa solo *rigenerazione dal default*. Per `koaSession` e `adminConfig` un file derivato dal default è pienamente utilizzabile — e nel caso peggiore, quello delle chiavi di sessione placeholder, il degrado **non è silenzioso**: `core/sessionSecurity.js → checkSessionKeys()` emette già il suo box di avviso al boot. Per `ital8Config` no: un file derivato dal default significherebbe un progetto mai configurato che parte in silenzio, scavalcando il wizard. Di qui il gate `[INIT]`, che resta.
  - **Due momenti diversi, non un ciclo unico:** `koaSession.json5` è letto dal montaggio dei priority middleware, che in `index.js` gira **a livello di modulo** (CommonJS, niente top-level await) — quindi va materializzato lì, con la variante sincrona `materializeFromDefault.sync`. `adminConfig.json5` è letto da `adminSystem.initialize()` e basta `startApp()`.
  - **Se manca anche il `.default`** non c'è nulla da cui rigenerare: è un'installazione incompleta → box `[CONFIG]` + `exit 1`, invece di un errore criptico più avanti o di un avvio a metà.
