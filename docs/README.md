...
- [`cli-control-plane.it.md`](./cli-control-plane.it.md) — control plane CLI (`ital8cms-cli`): pilotare un'istanza in esecuzione via SSH (attivare/disattivare l'area admin, manutenzione del sito pubblico, chiusura della superficie riservata per un sito vetrina, reset config)
- [`backup-strategie.it.md`](./backup-strategie.it.md) — le **due** strategie di backup che convivono sotto `backups/`: lo snapshot a comando (`npm run backup`) e il backup automatico del wizard di installazione, con i limiti noti di quest'ultimo e come recuperare a mano
